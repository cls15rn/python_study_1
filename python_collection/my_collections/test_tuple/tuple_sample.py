# path: my_collection\test_tuple\tuple_sample.py
# module: my_collection.test_tuple.tuple_sample
...
def tuple_indexing():
    print(f'{lst}, type: {type(lst)}')
    print(f'{tpl}, type: {type(tpl)}')

    print('0번째 앨리먼트:', lst[0], tpl[0])
    print('0번째 부터 1번째 까지의 데이터들:', lst[0:2], tpl[0:2])
...
def tuple_caution():
    ...
    tpl_3 = 1,
    print(f'tpl_3: {tpl_3}, type: {type(tpl_3)}') # tpl_3: (1,), type: <class 'tuple'>
    ...
    # return 원칙: 값 1개만 함수 실행 맨 마지막에 딱 한번 실행되어야 함
...
def tuple_builtin():
    ...
    print(f'tpl에 저장된 33의 순번: {find_index}') # 2

    # len(튜플변수): 저장된 데이터 갯수 조회
    print(f'tpl에 저정된 데이터 갯수: {len(tpl)}') # 3
    return
