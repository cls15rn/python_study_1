...
def list_remove():
    ...
    print(f'before lenght: {len(lst_1)}')
    
    lst_1.remove(1.2) # 앞에서부터 검색해서 첫번째로 만나는 값을 삭제함
...
def list_sort():
    ...
    lst_int = [5, 3, 6, 8, 9, 278, 12, 4, 53, 1, 7 ]
    ...
    lst_str = ['orange', 'banana', 'melon', 'apple', 'watermelon']
    ...
    lst_kr = ['파이썬', '데이터베이스', '웹크롤링', '데이터처리', '데이터분석', '초거대언어모델', '데이터처리']
...
def list_count():
    print(f'[{lst}')
    print(f'lst에 저장된 정수 1의 갯수 : {lst.count(1)}')
    return
