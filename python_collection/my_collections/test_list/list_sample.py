...
# 함수들만 저장되어 잇는 소스 파일 == 모듈(module) 이라고 함
...
저장 용량에 제한이 없음
저장되는 값의 종류에도 제한이 없음
저장 순서에 대한 순번(인덱스, index)가 있음 => 인덱싱, 슬라이싱 연산
'''
...
# lsit 자료형 특징 1: 문자열(str)과 같이 인덱싱, 슬라이싱 연산이 가능함
# index(순번: 저장 순서)
# 인덱싱 표현: 리스트 변수[순번]
def list_indexing():
...
def list_slicing():
    lst = [1, 2, 3, 'python', 3.45, [11, 22, 33], True, False]
    print('lst에 저장된 데이터 갯수:', len(lst)) # 8
    # len()을 이용해서 마지막 위치의 값 조회에 활용할 수도 있음
    print('lst에 저장된 마지막 인덱스 조회:',len(lst) - 1) # 7
    print('lst의 마지막 값:', lst[len(lst) - 1], lst[-1]) # False False
    print('lst의 1번째부터 마지막 인덱스 앞 위치까지 부분 추출:', lst[1:len(lst)-1:1]) # [2, 3, 'python', 3.45, [11, 22, 33], True] => list
    
    print('0번 인덱스부터 3번 인덱스까지 데이터 추출:', lst[0:4:1]) # [1, 2, 3, 'python'] ->  list로 변환됨
...
def change_element():
    lst = [1, 2, 3, 'python', 3.45, [11, 22, 33], True, False] 
    # 지역변수(localvariable): 함수 안에서 만든 변수
