...
# 동일한 시퀀스 자료형인 리스티(list), 튜플(tuple), 배열(arrary), 시리즈(Series)별로 동일하게 적용됨

# 문자 선택 연산자 (인덱싱): 문자열변수[인덱스순번]
ss = 'Hi,-python'

print('첫번째 글자:', ss[0]) # H
print('다섯번재 글자(인덱스 4):', ss[4]) # p
print('뒤에서 부터 첫번째(마지막 글자):', ss[-1]) # n
...
print(ss)
print(ss[::-1]) # 문자열 전체임, 간격이 음수이면 뒤에서부터 추출임: nohtyp-,iH

# 슬라이싱과 연결 연산(+)을 혼합해서 사용 가능
n1 = 'abcdef'
n2 = '12345'
n3 = n1[0:3] + n2[1:]
print(n3) #abc2345
...
print('|', tt4, '|', sep="") # sep='' 출력값들 사이의 공백 구분자 없엤음
print('|', tt4.strip(), '|', sep="") # 문자열 앞 뒤 공백 제거함
print('|', tt4.lstrip(), '|', sep="") # 문자열 앞(왼쪽) 공백 제거함
print('|', tt4.rstrip(), '|', sep="") # 문자열 뒤(오른쪽) 공백 제거함
